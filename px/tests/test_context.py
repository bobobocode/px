...
from px.context import setup_http_process_module, setup_wsgi_process_module
...
class TestSetupFunctions(unittest.TestCase):
    ...
    def test_setup_http_process_module_with_valid_module(self):
        with patch('importlib.import_module') as mock_import_module:
            mock_module = MagicMock()
            mock_import_module.return_value = mock_module

            setup_http_process_module('my_module.some_module')
            self.assertIsNotNone(PxContext.request_process_module)
    ...
    def test_setup_http_process_module_with_invalid_module(self):
        with self.assertRaises(ValueError):
            setup_http_process_module('invalid_module_name')

    def test_setup_wsgi_process_module_with_valid_module(self):
        with patch('importlib.import_module') as mock_import_module:
            mock_app_module = MagicMock()
            mock_app_module.wsgi_app = MagicMock(return_value=lambda env, start_response: None)
            ...
            self.assertIsNotNone(PxContext.request_process_module)
            self.assertEqual(PxContext.request_process_module.__name__, 'px.wsgi_process_module')
