...
from px.http import generate_response
from px.http import parse_http_headers
from px.http import parse_http_request_line


class TestHttpMethods(unittest.TestCase):
    def test_parse_http_request_line_valid(self):
        self.assertEqual(
            parse_http_request_line(b"GET /index.html HTTP/1.1"),
            ("GET", "/index.html", "HTTP/1.1"),
        )
        self.assertEqual(
            parse_http_request_line(b"POST /login HTTP/1.0"),
            ("POST", "/login", "HTTP/1.0"),
        )
